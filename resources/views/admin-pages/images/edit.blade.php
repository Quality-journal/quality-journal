<x-app-layout>
    ...
    <x-slot name="title">{{ 'Izmena slike' }}</x-slot>

    <main class="w-full flex-grow p-6">

        <div class="flex justify-between mb-2">
            <p class="text-xl pb-4">
                <i class="fas fa-plus mr-2"></i> Izmeni sliku
            </p>
            <span class="pb-4">
        ...
        <div class="leading-loose">
            <form class="p-10 bg-white rounded shadow-xl" method='POST' action="{{route('photos.update', $image)}}" enctype='multipart/form-data'>
                @csrf
                @method('PUT')
                ...
                <div class="mt-5">
                    ...
                    <input class="w-full sm:w-1/5 px-5 py-2 text-gray-700 bg-gray-200 rounded border-none" id="position" name="position" type="number" min="1"
                        aria-label="position" value="{{ $image->position }}">
                </div>

                <div class="mt-5">
                    <label class="block text-gray-600 mb-1" for="image">Image</label>
                    <input class="w-full sm:w-1/4 px-5 py-2 text-gray-700 bg-gray-200 rounded border-none" id="image" name="image" type="file"
                        aria-label="image" accept="image">
                </div>
