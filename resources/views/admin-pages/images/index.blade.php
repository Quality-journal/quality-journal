<x-app-layout>
    ...
    <main class="w-full flex-grow p-6 min-h-screen">
        ...
        <div class="w-full sm:w-1/6">
            <a href="{{ route('photos.create') }}" class="w-full bg-white cta-btn font-semibold py-2 mt-2 rounded-br-lg rounded-bl-lg rounded-tr-lg shadow-lg hover:shadow-xl hover:bg-gray-300 flex items-center justify-center">
                        ...
                        <div class="bg-white px-4 py-6 rounded-lg">
                            <div class='inline-flex space-x-4 float-right'>
                                ...
                                <form action="{{ route('photos.destroy', $image)}}" method="POST" id="deleteForm-{{ $image->id }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" onclick="confirmDelete(event, '{{ $image->id }}')"><svg xmlns="http://www.w3.org/2000/svg" class="text-red-600 h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
